    ...
    class Indicators(bt.Strategy):
        def __init__(self):
            for data in self.datas:
                ...
                data.red_candles = visualizers.SingleMarker(signals=signal_line, level=data.low*1.08, plotmaster=data, color='red', markersize=6, plot=True)
                data.no_wick_candles = visualizers.PartialLevel(signal=signal_line, level=data.high*1.07, color='yellow', plotmaster=data, plot=True)
