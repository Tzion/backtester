...
from database import diff_data_feed, feed_to_dataframe
import functools
...
def connectivity_warning(test):
    @functools.wraps(test)
    def test_wrapper(self, *args, **kwargs):
        try:
            test(self, *args, **kwargs)
        except Exception as e:
            raise Warning('Make sure IB Gateway or TWS is connected') from e
    return test_wrapper
...
class TestIBLoader:
    ...
    def test_request_feed_data_for_period(self, cerebro, loader, start, end, mocker):
        ...
        requested_df = feed_to_dataframe(requested_data)
    ...
    def test_request_feed_data_on_weekend(self, cerebro, loader):
        ...
        loader.load_feeds(['NVDA'], datetime(2021,11,15), datetime(2021,11,22), backfill_from_database=False, store=False)
        cerebro.addstrategy(test_common.DummyStrategy)
        cerebro.run()
